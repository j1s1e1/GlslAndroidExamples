...
public class Tut_Blender extends TutorialBase {
    ...
    public void display()
    {
       ...
       GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT |  GLES20.GL_DEPTH_BUFFER_BIT);
        blender.Draw();
        blender2.Draw();
        xoffset = 0.1f * (float) (Math.cos(anglevertical) * Math.cos(anglehorizontal));
        yoffset = 0.1f * (float) (Math.cos(anglevertical) * Math.sin(anglehorizontal));
        zoffset = 0.1f * (float) (Math.sin(anglevertical)) / 10f;
